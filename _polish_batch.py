"""One-shot polish for q-t articles: add opening CalloutBox + closing sig block."""
import os
import re
import sys
...
def is_in_range(name: str) -> bool:
    return name.lower().startswith(("q", "r", "s", "t")) and name.endswith(".tsx")
...
def add_opening_disclaimer(text: str, slug: str) -> tuple[str, bool]:
    ...
    # Find `return (` then `<>` on next non-empty line.
...
def add_closing_sig(text: str, slug: str) -> tuple[str, bool]:
    ...
    # Find last occurrence of `</>` so we cover anomalies.
    idx = text.rfind("</>")
    if idx == -1:
    ...
    # The closing block should be inserted at line_start so it appears just before </>.
    new_text = text[:line_start] + CLOSING_BLOCK + text[line_start:]
    return new_text, True
...
def main():
    ...
    print("## Polish batch q-t: completed")
