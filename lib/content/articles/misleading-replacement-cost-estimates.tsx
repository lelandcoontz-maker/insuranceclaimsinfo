...
export default function Content() {
  return (
    <>
      ...
      <h2 className="text-2xl font-bold text-[#1F3964] mb-4">
        The Tool Behind the Number: 360Value and Similar Estimators
      </h2>
      <p className="text-gray-700 leading-relaxed mb-4">
        The dwelling limit on your policy usually did not come from a person walking your
        property. It came from a software tool the insurer or agent ran at underwriting. One of
        the most widely used is 360Value, a replacement-cost estimator sold by Verisk Analytics
        &mdash; the same company behind Xactimate, the platform many carriers use to price claims.
        Others, including CoreLogic and e2Value, work the same way. You feed the tool a set of
        property characteristics &mdash; square footage, construction quality, number of stories,
        roof type, notable features &mdash; and it returns a rebuild-cost figure. That figure, more
        often than not, becomes your dwelling limit.
      </p>
      <p className="text-gray-700 leading-relaxed mb-4">
        The trouble starts with the inputs. A replacement-cost tool is only as good as the data
        entered into it, and the data is frequently rough: square footage pulled from a tax record
        and rounded, a custom home flagged as generic &ldquo;average&rdquo; or
        &ldquo;standard&rdquo; quality, upgraded finishes and structural features left out
        entirely. Layer in regional cost assumptions that may not reflect your specific market and
        the near-universal absence of an actual site visit, and the number the tool produces can
        drift well below what a contractor would quote. The homeowner, seeing a figure generated by
        the carrier&apos;s own system, accepts it &mdash; and only learns at a total loss that it
        will not rebuild the house.
      </p>
      <p className="text-gray-700 leading-relaxed mb-4">
        It is worth being fair about where the fault lies. 360Value and tools like it are not
        inherently wrong, and they can produce reasonable figures when they are fed accurate,
        complete inputs and checked against real construction costs. The problem is rarely the
        software itself. It is the unverified reliance on a tool-generated number &mdash; treating
        a quick automated estimate as though it were a considered appraisal of what your home would
        cost to rebuild. That is the same underinsurance dynamic described throughout this article,
        viewed one level deeper: at the specific point where the limit is first set.
      </p>
      <p className="text-gray-700 leading-relaxed mb-4">
        California&apos;s claims regulations speak to this kind of reliance. Under the Fair Claims
        Settlement Practices Regulations, an insurer&apos;s use of information or data supplied by a
        third party &mdash; which is exactly what an outside estimating tool provides &mdash; does
        not, by itself, relieve the insurer of its own regulatory responsibilities. See 10 CCR
        &sect; 2695.1(g). In plain terms, an insurer generally cannot point to a vendor&apos;s
        software as the reason a limit was inadequate and treat that as the end of the inquiry.
      </p>
      <p className="text-gray-700 leading-relaxed mb-4">
        None of this means the carrier is automatically responsible for the dollar gap between a
        tool-generated limit and the true cost to rebuild. Whether an insurer, an agent, or a
        broker bears legal responsibility for an inadequate limit is a fact-specific question, and
        the law on an agent&apos;s duty to advise is far from settled. If you are facing a
        shortfall that traces back to a tool-generated estimate, that question is worth raising with
        a licensed California attorney and reviewing alongside our discussion of{' '}
        <Link href="/resources/broker-agent-liability" className="text-[#2E74B5] hover:underline">
          broker and agent liability
        </Link>{' '}
        &mdash; rather than assuming any single party is liable.
      </p>
