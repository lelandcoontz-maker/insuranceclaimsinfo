...
export function AccordionSection({ id, title, children, defaultOpen = false }: AccordionSectionProps) {
  ...
  const buttonId = `${id || generatedId}-btn`
  ...
  return (
    <div id={id} className="border border-gray-200 rounded-lg overflow-hidden">
      <button
        ...
        className="w-full flex items-center justify-between px-5 py-4 text-left bg-white hover:bg-gray-50 transition-colors focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#1F3964]"
      >
      ...
      {open && (
        <div
          ...
          className="px-5 pb-5 pt-2 text-sm text-gray-700 leading-relaxed border-t border-gray-100"
        >
          {children}
        </div>
      )}
